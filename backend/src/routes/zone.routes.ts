 ...
 *         description: Created
 */
router.post('/', verifyToken, authorize('ADMIN'), validate(createZoneSchema), async (req, res, next) => {
  try {
    const { name } = req.body;
 ...
 *         description: Updated
 */
router.put('/:id', verifyToken, authorize('ADMIN'), validate(createZoneSchema), async (req, res, next) => {
  try {
    const { name } = req.body;
