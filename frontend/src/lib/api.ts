...
export const apiService = {
  ...
  orders: {
    ...
    assignAgent: (id: string, agentId: string) => api.post(`/orders/${id}/assign`, { agentId }),
    autoAssign: (id: string) => api.post(`/orders/${id}/auto-assign`),
    updateStatus: (id: string, status: string, notes?: string) => api.patch(`/orders/${id}/status`, { status, notes }),
    reschedule: (id: string) => api.post(`/orders/${id}/reschedule`),
    overrideStatus: (id: string, status: string) => api.patch(`/orders/${id}/override`, { status }),
  },
  zones: {
  ...
  rateCards: {
    ...
    updateRateCard: (id: string, data: any) => api.put(`/rate-cards/${id}`, data),
    deleteRateCard: (id: string) => api.delete(`/rate-cards/${id}`),
  },
  agents: {
    getAgents: () => api.get('/agents'),
    createAgent: (data: any) => api.post('/agents', data),
    updateLocation: (lat: number, lng: number) => api.patch('/agents/location', { lat, lng }),
    toggleAvailability: () => api.patch('/agents/availability'),
    getAgentOrders: () => api.get('/agents/orders'),
  },
  tracking: {
